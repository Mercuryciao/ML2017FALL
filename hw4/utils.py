...
class DataManager:
    ...
    '''
    def add_test(self, name, data_path):
        print ('read data from %s...'%data_path)
        X=[]
        with open(data_path,'r') as f:
            for line in f:
    ...
    '''                                                                             
    def wordvec(self):
        sentences = []#self.data['train_data'][0]
        #print(sentences)
        for key in self.data:
            for words in self.data[key][0]: 
                sentences.append(words.split())
        
        model = Word2Vec(sentences,window = 9, min_count = 170, sg = 1)
    ...
    def clean(self):
        for key in self.data:
            ...
            for index,words in enumerate(self.data[key][0]):
                words = words.replace("n't", ' not')
                words = words.replace("n ' t", ' not')
                words = words.replace("\n", '')
                words = words.replace("'ll",'will')
                words = rule.sub('',words)
                words = shortword.sub('',words)
                words =  re.sub(r'(.)\1+',r'\1\1',words)
                words = words.replace(" ca ", ' can ')
                words = words.replace(" dam ", " damn ")
                words = words.replace("damned", 'damn')
                words = words.replace(" ll","will")
                words = words.replace("wi fi","wifi")

                self.data[key][0][index] = words

            print(self.data[key][0][34:44])
